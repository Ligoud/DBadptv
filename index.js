...
app.get('/admin/themes',(req,res)=>{
    console.log(pg.getQuestionThemes())
    res.send(JSON.stringify({
        arr:['a','b','c']
    }))
})
app.get('/admin/themes/:nameTheme',(req,res)=>{
    res.send('yeiks')
})
app.get('/admin/checkfiles',async (req,res)=>{
    var result=await parser.checkFiles()
    for(let i=0;i<result.length;i++){
        pg.addQuestion(result[i].theme,result[i].level ,result[i].quest,result[i].cases,result[i].answ)        
    }
    //console.log(result)
    res.send('true')
})
